---
# Display name
title: Katie Burak

# Name pronunciation (optional)
name_pronunciation: 

# Full name (for SEO)
first_name: Katie
last_name: Burak

# Status emoji
status:
  icon: 

# Is this the primary user of the site?
superuser: true

# Highlight the author in author lists? (true/false)
highlight_name: true

# Role/position/tagline
role: Assistant Professor of Teaching

# Organizations/Affiliations to display in Biography blox
organizations:
  - name: University of British Columbia
    url: https://www.stat.ubc.ca/users/katie-burak

# Social network links
# Need to use another icon? Simply download the SVG icon to your `assets/media/icons/` folder.
profiles:
  - icon: at-symbol
    url: 'mailto:kburak@stat.ubc.ca'
    label: E-mail Me
  - icon: brands/github
    url: https://github.com/katieburak
  - icon: brands/linkedin
    url: https://www.linkedin.com/in/katie-burak-a9a12615b/

interests:
  - Data Science
  - Statistics Education and Outreach
  - Equity, Diversity and Inclusion 

education:
  - area: PhD in Statistics
    institution: University of Alberta
    date_start: 2019-09-01
    date_end: 2023-04-30
    
  - area: MSc in Statistics
    institution: University of Calgary
    date_start: 2017-09-01
    date_end: 2029-06-30
    
  - area: BSc in Mathematics
    institution: MacEwan University
    date_start: 2012-09-01
    date_end: 2017-04-30

  
work:
  - position: Assistant Professor of Teaching
    company_name: Department of Statistics, UBC
    company_url: ''
    company_logo: ''
    date_start: 2023-07-01
    date_end: ''


---

## About Me

Katie Burak is an Assistant Professor of Teaching in the Department of Statistics at the University of British Columbia. Her work focuses on modern pedagogical methods in statistics and data science. She is passionate about statistical outreach and education, organizing various events in her community. Katie is particularly dedicated to advancing women in data science and addressing equity issues within the statistical sciences.
